...
const PKG_RE = /(?:react\-material\-components)|(?:[a-zA-Z\-]+)/;

const isValidCwd = (
    path.basename(process.cwd()) === 'react-web-workspaces-blueprint' &&
...
if (!isValidCwd) {
    console.error(
        ...
        'and that you have run `yarn dist`'
    );
    process.exit(0);
}
...
function cpAsset(asset) {
    ...
    const nameParts = path.basename(asset).split('.');
    const minExt = nameParts[nameParts.length - 2] === 'min' ? '.min' : '';

    const destDir = path.join(assetPkg, 'dist', 'index' + minExt + path.extname(asset))
